...
function getYAxisTextList(series, opts, config2, stack, yData) {
  ...
  var maxRange = yData.max === void 0 || yData.min === null ? dataRange.maxRange : yData.max;
  var range = [];
...
function drawPieText(series, opts, config2, context, radius, center) {
  ...
  var seriesConvert = series.map(function(item, index, series2) {
    var text = item.formatter ? item.formatter(item, index, series2) : util.toFixed(item._proportion_.toFixed(4) * 100) + "%";
...
function drawMarkLine(opts, config2, context) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (item.showLabel) {
      ...
      context.setFontSize(config2.fontSize);
      let textWidth = measureText(labelText, config2.fontSize, context);
      opts.chartData.yAxisData.yAxisWidth[0].width;
      let bgStartX = opts.area[3] - textWidth - config2.toolTipPadding * 2;
      let bgEndX = opts.area[3];
      let bgWidth = bgEndX - bgStartX;
      let textX = bgEndX - config2.toolTipPadding;
      let textY = item.y;
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config2._xAxisTextAngle_ === 0) {
      ...
      newCategories.forEach(function(item, index) {
        ...
        var _calRotateTranslate = calRotateTranslate(xAxisPoints[index] + eachSpacing / 2, startY + xAxisFontSize / 2 + 5, opts.height), transX = _calRotateTranslate.transX, transY = _calRotateTranslate.transY;
